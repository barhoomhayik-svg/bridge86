...
import {
  ...
  ChevronRight, TrendingUp, Plus, Search, Microscope, X,
  Heart, MapPin, FlaskConical,
} from 'lucide-react';
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">Your orders and activity</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <StatCard icon={<ClipboardList className="w-5 h-5 text-primary-600" />} bg="bg-primary-50" value={orders.length} label="Total Orders" />
        <StatCard icon={<TrendingUp className="w-5 h-5 text-blue-600" />} bg="bg-blue-50" value={activeCount} label="Active" />
        <StatCard icon={<CheckCircle2 className="w-5 h-5 text-teal-600" />} bg="bg-teal-50" value={counts['completed'] ?? 0} label="Completed" />
        <StatCard icon={<XCircle className="w-5 h-5 text-red-400" />} bg="bg-red-50" value={(counts['rejected'] ?? 0) + (counts['cancelled'] ?? 0)} label="Closed" />
      </div>
            ...
            <>
              ...
              <div className="flex items-center justify-center gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-neutral-300 text-neutral-700 hover:bg-neutral-50 text-sm font-semibold rounded-xl transition-colors"
                >
      ...
      {orderTarget && (
        <NewOrderModal
          ...
          onSuccess={() => { setOrderTarget(null); fetchOrders(); }}
        />
      )}
    </div>
  );
...
function FavouriteLabPickerModal({
  onSelect, onClose, onBrowseAll,
}: {
  onSelect: (lab: Profile) => void;
  onClose: () => void;
  onBrowseAll: () => void;
}) {
  const { profile } = useAuth();
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-modal w-full max-w-md max-h-[82vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-neutral-200">
          <div>
            <h2 className="font-bold text-neutral-900">Select a Lab</h2>
            <p className="text-sm text-neutral-500">
              {favouriteIds.size > 0 ? 'Your favourites are shown first' : 'Choose the lab to place an order with'}
            </p>
          </div>
            ...
            <>
              ...
              {favourites.length > 0 && (
                <div className="pt-3">
                  <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                    <Heart className="w-3 h-3 fill-red-400 text-red-400" /> Favourites
                  </p>
                  <div className="space-y-1">
                    {favourites.map(lab => <LabRow key={lab.id} lab={lab} isFav onSelect={onSelect} />)}
                  </div>
                </div>
              )}
              ...
              {others.length > 0 && (
                <div className={favourites.length > 0 ? 'pt-4' : 'pt-3'}>
                  {favourites.length > 0 && (
                    <p className="text-[10px] font-semibold text-neutral-400 uppercase tracking-wider mb-2">All Labs</p>
                  )}
                  <div className="space-y-1">
                    {others.map(lab => <LabRow key={lab.id} lab={lab} isFav={false} onSelect={onSelect} />)}
                  </div>
                </div>
              )}
            </>
        ...
        <div className="px-5 pb-5 pt-2 border-t border-neutral-100">
          <button
            ...
            className="w-full py-2.5 border border-neutral-300 text-neutral-600 hover:bg-neutral-50 text-sm font-semibold rounded-xl transition-colors flex items-center justify-center gap-2"
          >
            <Search className="w-4 h-4" /> Browse all labs
          </button>
        </div>
      </div>
    </div>
